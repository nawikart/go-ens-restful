...
func setSubdomainOwner(data Data) Result {
	var result Result
	...
	_, err = registry.SetSubdomainOwner(opts, data.DomainName, data.SubDomain, common.HexToAddress(data.ToAddress))
	if err != nil {
		result.Status = "failed"
		result.ErrorMsg = fmt.Sprint(err)
		return result
...
func SetSubdomainOwner(w http.ResponseWriter, r *http.Request) {
	body, _ := ioutil.ReadAll(r.Body)
	var data Data
	if json.Unmarshal(body, &data) == nil {
		json.NewEncoder(w).Encode(setSubdomainOwner(data))
		return
	}
